...
$jautNR = intval($_GET['question_id']); // iegūst jautājuma id no DB
$jautTeksts = $visiJaut[$jautNR]['question_text']; //iegūst jautājuma tekstu no DB
$stmnt = $db->prepare("select id,answer_text FROM answers WHERE question_id=?"); // iegūst atbildes attiecīgajam jautājumam
...
if ($jautNR == 0) {
    ...
    $stmnt->execute();
    setcookie('user_id', mysqli_insert_id($db)); // saglabā lietotāja ID no DB
} else {

    //ieliek atbildi DB

    $atbId = intval($_GET['answer_id']);
    ...
    $stmnt->bind_param("iii", $userId, $questionId, $atbId);
    $stmnt->execute();
}
mysqli_close($db);
...
<div id="Lapa1_1">
    ...
    <?php
    $progrVert = 100 / (intval($visiJaut) + 1); //progress bar vienas iedaļas aprēķināšana
